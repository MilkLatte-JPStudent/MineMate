      ...
      saveSpatialMemory: (topic, route, coords, screenshotFilename) => {
        try {
          ...
          if (bot && bot.entity) {
             ...
             finalCoords = `${coords} (Yaw: ${yaw.toFixed(2)}, Pitch: ${pitch.toFixed(2)})`;
          }
          ...
          expData.experiences.push({ topic, route, coords: finalCoords, screenshotFilename, timestamp: new Date().toISOString() });
          fs.writeFileSync(expPath, JSON.stringify(expData, null, 2), 'utf8');
